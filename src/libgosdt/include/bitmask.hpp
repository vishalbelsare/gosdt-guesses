...
typedef unsigned short rangeblock;  // Type used to chunk the binary bits into
                                    // precomputable sequences
typedef char codeblock;             // Type used to store run-length codes for each
                                    // precomputable sequence

// This declaration acts as both a function module and a container class
// The static class methods implements a function module providing operations on
// arrays of type bitblock, which can be allocated on the stack The non-static
// class methods implementes a heap-allocated equivalent, which supports the
// same methods
// @note: Many of the binary operations assume that operands have the same
// length
class Bitmask {
   public:
    static const bitblock ranges_per_code;  // Number of ranges that can be
                                            // encoded using a single codeblock
    ...
    // @param number_of_blocks: pointer to store the number of blocks required
    // to represent these bits
    // @param block_offset: pointer to store the number of bits actually used in
    // the last block
    // @modifies number_of_bits: initialized with the number of blocks required
    // to represent these bits
    // @modifies block_offset: initialized with the number of bits actually used
    // in the last block
    ...
    // @param blocks: the blocks from which to compute the number of contiguous
    // ranges of bits which are set to 1
    // @param size: the number of bits which are represented in blocks
    // @returns the estimated number of contiguous ranges of bits which are set
    // to
    // 1
    // @note the this was originally an approximate method. I later altered this
    // to an exact method :D
    static unsigned int words(bitblock *blocks, unsigned int size);

    // @param blocks: blocks which act as a vector of bits for a bit-wise
    // logical and
    // @param other_blocks: blocks which act as a vector of bits for a bit-wise
    // logical and
    // @param size: the number of bits which are represented in blocks and
    // other_blocks
    // @param flip: whether or not to interpret the bits of blocks as flipped
    // before applying the bit-wise operation
    ...
    // @note this comparison uses bit 0 as the least significant bit (little
    // endian)
    ...
    // @modifies blocks: blocks will be modified at the specified index with the
    // specified value
    ...
    // @param size: the number of bits which are represented in blocks
    // @param start: the starting index (inclusive) to scan for a bit that
    // matches 'value'
    ...
    // @returns a string containing the bits in order of least significant to
    // most significant
    static std::string to_string(bitblock *blocks, unsigned int size, bool flip = false);

    static bool integrity_check;                        // Flag that indicates whether null pointer
                                                        // checks are performed on object instances
    ...
    static void precompute(void);              // Perform the one-time precomputation for
                                               // rangeblock-sized sequences
    ...
    // @modifies dest_blocks: the contents of dest_blocks will be overwritten
    // with a copy of the bits stored in this bitmask
    void copy_to(bitblock *dest_blocks) const;

    // @param dest_blocks: an array of bitblocks to copy bits from
    // @modifies content: the instance member content will be overwritten with
    // _size blocks of src_blocks
    ...
    // @returns the index of the first bit that matches value, starting at
    // (inclusive) the index 'start'.
    // @note scan returns size if scan doesn't find any matches, -1 if rscan
    // doesn't find any matches
    // @note the scan implementation searches from start to most-significant bit
    // @note the rscan implementation searches from start to least-significant
    // bit
    ...
    // @modifies begin: the index of the first bit in the scan that matches
    // value
    // @modifies end: the index of the first bit that doesn't match the value
    // following the bit matched at 'begin'
    // @return true if a range is found
    // @note these implementations are for finding contiguous ranges of bits
    ...
    // @param flip: whether or not to treat the bits of this instance as flipped
    ...
    // @param other: a bitmask containing an array of bitblocks containing bits
    // to be compared
    // @returns whether the bits match up to the size of this instance
    ...
    // @note the 0th bit is considered most significant when making this
    // comparison
    bool operator<(Bitmask const &other) const;
    ...
    // @returns: the depth budget for the subproblem this bitmask represents
    // (assuming the bitmask represents a subproblem)
    unsigned char get_depth_budget() const;

    // @param: depth_budget: the depth budget for the subproblem this bitmask
    // represents.
    void set_depth_budget(unsigned char depth_budget);

    // @param reverse: reverses the sequence so that the 0th bit is on the
    // right, as opposed to on the left (little endian)
    // @returns a string representing the bit sequence with each bit represented
    // as the character '1' or '0'
    // @note default reverses the sequence so that bit-0 is the left most in the
    // string
    ...
    // @requires the new_size must be less that _max_blocks * bits_per_block.
    // (i.e. the maximum capacity set during construction)
    // @modifies _size: modified to the new size
    // @modifies _used_blocks: modified to the new number of used blocks
    // @modifies _offset: modified to the new number of bits used in the last
    // used block
    // @note: resizing does not affect memory usage. allocated memory remain
    // constant until this instance is deconstructed
    void resize(unsigned int _new_size);  // The number of bits actually being used
                                          // (excludes leading zeros in final block)

    friend std::ostream &operator<<(std::ostream &out, const Bitmask &b);
    void print_per_block_count() const;

   private:
    unsigned char depth_budget;  // If Bitmask represents subproblem - maximum allowable
                                 // depth for the solution 1 means no further splits
                                 // allowed
                                 // - a (sub)tree with only one node is depth 1. 0 means
                                 // there is no constraint on allowable depth
    ...
    unsigned int _size = 0;                              // The number of bits actually being used (excludes
                                                         // leading zeros in final block)
    unsigned int _offset = 0;                            // The number of bits used in the last used block
    ...
    // If shallow is true, then the destructor is not responsible for
    // deallocating the memory pointed to by content
    bool shallow = false;
};
