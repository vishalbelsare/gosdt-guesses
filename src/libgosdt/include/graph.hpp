...
// These delegate to the already implemented hash functions and equality
// operators
class GraphVertexHashComparator {
   public:
    static size_t hash(key_type const &key) { return key.hash(); }
    static bool equal(key_type const &left, key_type const &right) { return left == right; }
};
...
typedef tbb::concurrent_hash_map<  // Table for storing vertices
    key_type, value_type, GraphVertexHashComparator, tbb::scalable_allocator<std::pair<key_type const, value_type>>>
    vertex_table;

typedef tbb::concurrent_unordered_map<  // Set of parents for a single vertex
    key_type, std::pair<Bitmask, float>, std::hash<Bitmask>, std::equal_to<Bitmask>,
...
typedef tbb::concurrent_vector<std::tuple<unsigned int, float, float>,
                               tbb::scalable_allocator<std::tuple<unsigned int, float, float>>>
    bound_list;  // List of split-bounds for a single vertex

typedef tbb::concurrent_hash_map<  // Table of all bound lists
    key_type, bound_list, GraphVertexHashComparator, tbb::scalable_allocator<std::pair<key_type const, bound_list>>>
    bound_table;
...
class Graph {
   public:
    ...
    // bool connect(key_type const & parent, key_type const & child, float
    // scope);
    ...
    // bool find(const_adjacency_accessor & accessor, key_type const & key, bool
    // forward = true) const; bool find(adjacency_accessor & accessor, key_type
    // const & key, bool forward = true) const;

    // bool find_or_create(const_vertex_accessor & accessor, key_type const &
    // key, Bitmask & buffer_1, Bitmask & buffer_2, Bitmask & buffer_3, Task
    // const & task, unsigned int index, bool condition);
